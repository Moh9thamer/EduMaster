...
namespace API.Controllers.Sections;
...
{
    [HttpGet]
    public async Task<IActionResult> GetAll([FromQuery] int gradeId, [FromQuery] int academicYearId)
    {
        var result = await service.GetByGradeAndYearAsync(gradeId, academicYearId);
        return Ok(result);
    }
