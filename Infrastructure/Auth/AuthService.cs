...
public class AuthService : IAuthService
{
    ...
    public async Task<Result> RegisterAsync(RegisterUserDto dto)
    {
        ...
        if (phoneExists)
            return Result.Fail("Phone number is already registered.");
    ...
    public async Task<Result<LoginResponseDto>> LoginAsync(LoginUserDto dto)
    {
        ...
        if (user == null)
            return Result<LoginResponseDto>.Fail("Invalid phone number or password.");
        ...
        return Result<LoginResponseDto>.Ok(new LoginResponseDto { Token = jwt, RefreshToken = refreshToken });
    }
    ...
    public async Task<Result<LoginResponseDto>> RefreshTokenAsync(string refreshToken)
    {
        ...
        if (stored == null || !stored.IsValid || stored.User == null)
        ...
        return Result<LoginResponseDto>.Ok(new LoginResponseDto { Token = jwt, RefreshToken = newRefreshToken });
    }
    ...
    public async Task<Result> RevokeTokenAsync(string refreshToken)
    {
        ...
        if (stored == null) return Result.Fail("Token not found.");
    ...
    public async Task<Result> ForgotPasswordAsync(ForgotPasswordDto dto)
    {
        ...
        if (user == null) return Result.Ok(); // don't reveal whether phone exists
        ...
        catch (Exception ex)
        {
            ...
            return Result.Fail("Failed to send reset code. Please try again later.");
        }
    ...
    {
        if (!await _otpService.ValidateAsync(dto.PhoneNumber, dto.Code))
            return Result.Fail("Password reset failed. Invalid or expired code.");
